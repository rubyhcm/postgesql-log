...
\begin{enumerate}
    ...
    \item \textbf{Giả mạo hash chain (sửa file binary):} Sửa file segment, chạy \texttt{func\_verify\_hash\_chain()} $\rightarrow$ phát hiện \texttt{chain\_ok = FALSE}, cảnh báo \texttt{HASH\_CHAIN\_BROKEN}. \textbf{PASS}.
\end{enumerate}
\end{sloppypar}
...
\midrule
Có GIN (warm cache) & Bitmap Heap Scan (2 partitions) và Seq Scan (5 partitions) & 930 ms \\
